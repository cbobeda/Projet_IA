...
std::vector<Node> Pathfinding::findPath(Grid& grid, sf::Vector2i start, sf::Vector2i end) {
    ...
    while (!openList.empty()) {
        ...
        for (sf::Vector2i& neighborPos : neighbors) {
            if (neighborPos.x < 0 || neighborPos.x >= GRID_WIDTH || neighborPos.y < 0 || neighborPos.y >= GRID_HEIGHT)
                continue;
            if (grid.cells[neighborPos.y][neighborPos.x].walkable == false || visited[neighborPos.y][neighborPos.x])
                continue;
