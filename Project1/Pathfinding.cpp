    ...
    while (!openQueue.empty()) {
        ...
        for (auto& dir : directions) {
            ...
            if (neighborPos.x < 0 || neighborPos.x >= GRID_WIDTH || neighborPos.y < 0 || neighborPos.y >= GRID_HEIGHT)
                continue; 
            if (!grid.cells[neighborPos.y][neighborPos.x].walkable)
                continue; 

            if ((dir.x != 0 && dir.y != 0) &&
                (!grid.cells[current->position.y][neighborPos.x].walkable || !grid.cells[neighborPos.y][current->position.x].walkable))
                continue;
